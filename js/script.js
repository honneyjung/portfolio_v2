  ...
  (function () {
    const MODAL_DATA = {
      hemily: {
        ...
        desc: "헬스케어 스타트업 해밀리의 신규 앱 프론트엔드 MVP 개발에 참여했습니다. 설문 기반 건강 유형 분류 화면부터 DNA 단계별 챌린지·리포트 UI까지 컴포넌트 설계 및 API 연동을 직접 담당했습니다. React 19·TypeScript를 선택해 타입 안정성을 확보하고 컴포넌트 단위 유지보수성을 높였습니다. 서버 상태와 클라이언트 상태를 분리하기 위해 TanStack Query로 API 캐싱·리페칭을 자동화해 반복적인 요청 로직을 줄였고, Zustand로 가벼운 전역 상태관리를 구성해 보일러플레이트를 최소화했습니다. 로그인 세션 만료로 인한 사용자 이탈을 방지하기 위해 JWT 자동 갱신과 Private Route 기반 접근 제어를 구현했습니다.",
      ...
      "carenation-web": {
        ...
        desc: "신규 서비스 추가에 따른 케어네이션 홈페이지 전면 리뉴얼을 담당했습니다. 페이지 구조 설계부터 퍼블리싱까지 단독으로 진행했으며, 기존 CSS를 SCSS로 리팩토링하여 유지보수성을 개선했습니다. 반복되는 CSS 구조로 인해 디자인 변경 요청이 들어올 때마다 여러 파일을 일일이 수정해야 하는 비효율이 있었습니다. SCSS 기반 공통 스타일 구조로 개선해 하나의 스타일 수정으로 여러 화면에 일괄 반영되도록 만들었고, 이를 통해 디자인 변경 대응 속도를 높이고 운영 중 UI 이슈 대응 부담을 줄였습니다.",
      ...
      "carenation-app": {
        ...
        desc: "요양·간병 매칭 서비스 케어네이션의 모바일 앱 UI를 퍼블리싱했습니다. 화면마다 버튼·폼·모달을 개별적으로 구현하면 신규 화면 추가 시마다 반복 작업과 디자인 불일치가 발생하는 문제가 있었습니다. 이를 해결하기 위해 공통 컴포넌트와 SCSS 기반 공통 스타일 구조를 설계했고, 그 결과 신규 화면 개발과 운영 단계에서 동일 UI를 재사용할 수 있게 되어 개발 공수와 유지보수 범위를 줄였습니다. 또한 Chrome·iOS Safari·Android 실기기 QA를 반복 수행해 배포 후 발생할 수 있는 UI 오류를 사전에 제거했습니다.",
      ...
      backoffice: {
        ...
        desc: "백오피스 경영진 전용 관리자 대시보드를 퍼블리싱했습니다. 테이블·차트·모달·폼 등 다양한 공통 컴포넌트를 구조화하고, 대규모 페이지 구성을 체계적으로 관리했습니다. 관리자 화면이 늘어날수록 테이블·검색·필터 등 유사 UI를 매번 새로 만들어야 하는 문제가 있었습니다. 공통 컴포넌트와 스타일 가이드를 표준화해 신규 화면 추가 시 기존 컴포넌트를 재사용할 수 있도록 했고, Git·Jira 기반으로 변경 이력과 이슈를 관리하며 기획·디자인·개발 간 협업 효율을 높였습니다.",
